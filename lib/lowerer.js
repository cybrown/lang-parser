...
var binaryToName = function (op) {
    ...
    if (!map.hasOwnProperty(op)) {
        throw new Error('Operator not found: ' + op);
    }
    return map[op];
};
...
Lowerer.prototype.InterfaceDeclarationEnter = function (node) {
    var classNode = nodes.ClassDeclaration(node.name, node.members);
...
Lowerer.prototype.UnaryExpressionLeave = function (node) {
    if (['!', '-', '~', '^'].indexOf(node.operator) !== -1) {
        return nodes.CallExpression(
            nodes.MemberExpression(node.argument, '$unary$' + node.operator),
            []
        );
    }
};
