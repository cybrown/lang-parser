...
describe ('Call', function () {
    ...
    it ('should parse an empty call', function () {
        ...
        assert.strictEqual(ast[0].callee.name, 'foo');
        assert.strictEqual(ast[0].arguments.length, 0);
    });
    ...
    it ('should parse a call with an argument', function () {
        ...
        assert.strictEqual(ast[0].callee.type, 'Identifier');
        assert.strictEqual(ast[0].callee.name, 'foo');
        assert.strictEqual(ast[0].arguments.length, 1);
    });
    ...
    it ('should parse a call with 3 arguments', function () {
        ...
        assert.strictEqual(ast[0].callee.type, 'Identifier');
        assert.strictEqual(ast[0].callee.name, 'foo');
        assert.strictEqual(ast[0].arguments.length, 3);
    });
});
