...
describe ('Lambda', function () {
    ...
    it ('should parse a lambda without arguments', function () {
        ...
        assert.strictEqual(ast[0].params.length, 0);
    ...
    it ('should parse a lambda with 1 argument without ()', function () {
        ...
        assert.strictEqual(ast[0].type, 'LambdaExpression');
        assert.strictEqual(ast[0].params.length, 1);
        assert.strictEqual(ast[0].params[0].type, 'Identifier');
        assert.strictEqual(ast[0].params[0].name, 'x');
        assert.strictEqual(ast[0].body.type, 'CallExpression');
    });
    ...
    it ('should parse a lambda with 2 arguments', function () {
        ...
        assert.strictEqual(ast[0].type, 'LambdaExpression');
        assert.strictEqual(ast[0].params.length, 2);
        assert.strictEqual(ast[0].params[0].type, 'Identifier');
        assert.strictEqual(ast[0].params[0].name, 'x');
        assert.strictEqual(ast[0].params[1].type, 'Identifier');
        assert.strictEqual(ast[0].params[1].name, 'y');
        assert.strictEqual(ast[0].body.type, 'CallExpression');
    });
});
